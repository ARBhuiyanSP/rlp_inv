...
if(isset($_POST['submit'])){
	...
	$resultSet = $conn->query("SELECT * FROM rlp_info WHERE rlp_user_id = $member".($status!='all'?" AND rlp_status = $status":'')." AND request_date BETWEEN '$fromdate' AND '$todate'");
	...
	if($resultSet->num_rows > 0){
		...
		while($rows = $resultSet->fetch_assoc()) {
			...
			echo "<tr><td>" . $i . "</td>
					...
					<td>" . $rows['email'] . "</td>
					<td>" . getNameByIdAndTable('department',$rows['request_department']) . "</td>
